...
alpha <- 0.05
delta <- 1
sig_antibiotic <- df_T0[df_T0$pval.antibiotic.fdr <= alpha, "OTUname"]
sig_genotype <- df_T0[df_T0$pval.genotype.fdr <= alpha, "OTUname"]
sig_genotype_antibiotic_inter <- df_T0[  df_T0$pval.interaction.fdr <= alpha, "OTUname"]
par(plotconf)
venn(list(Antibiotic = sig_antibiotic, Genotype = sig_genotype, Interaction = sig_genotype_antibiotic_inter))
# grid.newpage()
# grid.draw(VennDiagram::venn.diagram(list(Genotype = sig_genotype,
#                                          Antibiotic = sig_antibiotic,
#                                          Interaction = sig_genotype_antibiotic_inter), NULL,             
#                                     category = c("Genotype", "Antibiotics", "Interaction"),
...
l <- list(Genotype = sig_genotype, Antibiotic = sig_antibiotic, Interaction = sig_genotype_antibiotic_inter)
elements <- lapply(l, length)
names <- names(elements)
par(plotconf)
par(mar = c(3, 5, 2, 1))
bp <- barplot(rev(as.numeric(elements)), names = rev(c("Genotype", "Antibiotics", "Interaction")), horiz = TRUE,
              xaxs = "i", xlim = c(0, 800), xlab = "Number of signifificant OTUs")
axis(2, at = bp, lab = NA)
mbox(aty = bp)
```
...
updown_T0_MinusA <- aggregate(abundance ~ family + OTUname,
                              FUN = mean, na.rm = TRUE, 
