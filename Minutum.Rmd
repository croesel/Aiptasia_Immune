...
alpha <- .05
logThreshold <- .05
dna <- readDNAStringSet("Minutum/minutumTrans.fa")
```

```{r}
tx2geneNoAnnot <- data.frame(Transcript=names(dna), Gene=names(dna))
tx2geneNoAnnot <- tx2geneNoAnnot %>% separate(Transcript, c("Transcript", "Gene"), " gene=")
tx2geneNoAnnot <- tx2geneNoAnnot %>% separate(Gene, c("Gene", "CDS"), " CDS=")
```
...
Aligned <- unlist(colSums(counts.salmon$counts))
...
```
```{r}
resMenthol <- results(ddsAllNoAnnot, alpha=.05, name="Menthol_Menthol_vs_NoMenthol")
dfMenthol <- data.frame(resMenthol)
dfMenthol <- subset(dfMenthol, padj < .05, select=c("log2FoldChange", "padj"))
kable(dfMenthol, caption="DE Genes, Menthol")
summary(resMenthol)
```

```{r}
resVibrio <- results(ddsAllNoAnnot, alpha=.05, name="Vibrio_Vibrio_vs_NoVibrio")
dfVibrio <- data.frame(resVibrio)
dfVibrio <- subset(dfVibrio, padj < .05, select=c("log2FoldChange", "padj"))
kable(dfVibrio, caption="DE Vibrio")
```

```{r}
resBoth <- results(ddsAllNoAnnot, alpha=.05, name="VibrioVibrio.MentholMenthol")
dfBoth <- data.frame(resBoth)
dfBoth <- subset(dfBoth, padj < .05, select=c("log2FoldChange", "padj"))
kable(dfBoth, caption="DE, Interaction")
```