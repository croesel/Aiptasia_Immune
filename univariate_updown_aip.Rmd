...
load("univariate_glm_t0_metagenomeSeq_quasipoisson.RData")
 ...
 counts_merged <- data.frame(treatment = rownames(t(counts_norm)),
                                t(counts_norm))
 counts_merged <- merge(data.frame(treatment = rownames(phenos), phenos),
                           counts_merged, all.y = TRUE)
 counts_merged_long <- reshape(counts_merged,
                                  times = colnames(counts_merged[, 8:ncol(counts_merged)]),
                                  varying = list(8:ncol(counts_merged)),
                                  idvar = c("treatment", "times", "antibiotic",
                                            "exposure_one", "exposure_two",
                                            "genotype", "tank"),
                                  v.names = "abundance",
                                  direction = "long")
 colnames(counts_merged_long)[8] <- "GeneName"
 merged_counts_merged_long <- merge(counts_merged_long, annots, all.x = TRUE)
